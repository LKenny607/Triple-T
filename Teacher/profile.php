...
if (!empty($avatar_filename)) {
    // Relative to this file, not the web root — works no matter what
    // folder the whole project ends up deployed under.
    $avatar_disk_path = __DIR__ . '/../Admin/uploads/' . $avatar_filename;
    if (file_exists($avatar_disk_path)) {
        $avatarPath = '../Admin/uploads/' . $avatar_filename;
    }
}
